...
export default function PostDetail() {
    ...
    return (
      <div className="max-w-2xl mx-auto py-20 text-center">
        <div className="bg-destructive/10 p-8 rounded-2xl border border-destructive/20 mb-6">
          <p className="text-destructive font-semibold text-lg">{error || "Post not found"}</p>
        </div>
  ...
  const imageUrl = post.imageUrl || "https://images.unsplash.com/photo-1499750310107-5fef28a66643?q=80&w=2670&auto=format&fit=crop";
  ...
  return (
    <article className="max-w-4xl mx-auto animate-fade-in pb-20">
      <Link to="/" className="inline-flex items-center text-muted-foreground hover:text-foreground transition-colors mb-8 font-medium">
      ...
      <header className="mb-10 text-center">
        ...
        {post.tags && post.tags.length > 0 && (
          <div className="flex justify-center gap-2 mt-4">
            {post.tags.map(tag => (
              <span key={tag} className="text-xs font-semibold px-2 py-1 bg-primary/10 text-primary rounded-md flex items-center">
      ...
      <section className="glass p-8 rounded-3xl mt-12">
        <h3 className="text-2xl font-heading font-bold mb-6">Comments ({comments.length})</h3>
        ...
        {user ? (
          <form onSubmit={handleCommentSubmit} className="mb-8">
            ...
            />
            {commentError && <p className="text-destructive text-sm mt-2">{commentError}</p>}
          ...
          <div className="bg-secondary/30 p-6 rounded-xl text-center mb-8 border border-border/50">
            ...
            <Link to="/login" className="inline-block bg-primary text-primary-foreground px-6 py-2 rounded-lg font-medium hover:bg-primary/90 transition-colors">
              Log In
            </Link>
          </div>
        )}
        ...
        <div className="space-y-6">
          {comments.length > 0 ? (
            comments.map((comment, index) => (
              <div key={comment._id || index} className="flex gap-4 p-5 rounded-2xl bg-secondary/10 border border-border/50">
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start mb-1">
                    <h4 className="font-semibold">{comment.author?.name || "Anonymous User"}</h4>
                    <span className="text-xs text-muted-foreground">
                      {new Date(comment.createdAt).toLocaleString("en-US", { year: "numeric", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                    </span>
                  </div>
